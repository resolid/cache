...
Thank you for considering contributing to **Resolid Cache**!  
Before you submit your contribution, please take a moment to read the following guidelines.

## How to contribute

Follow these steps to contribute to **Resolid Cache**:
...
```shell
  git clone https://github.com/<your-username>/resolid-cache.git
  cd resolid-cache
```
...
The **Resolid Cache** follows the principles of Clean Code to ensure that code remains simple, readable, and maintainable.
...
```
feat(cache): add LazyResolve support for async factory
fix(cache-file): handle null input gracefully
...
```
refactor(cache): remove old container API
...
fix(cache). # Subject should not end with a period
docs: # Subject must not be empty
perf(cache): Improve speed # Avoid sentence-case / start-case
```
